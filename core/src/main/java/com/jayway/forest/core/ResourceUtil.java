...
import com.jayway.forest.reflection.ResourceMethod.MethodType;
...
public class ResourceUtil {
    ...
    public ResourceMethod makeResourceMethod(Resource resource, Method method) {
    	MethodType type = MethodType.NOT_FOUND;
        ...
        if (!Modifier.isAbstract(method.getModifiers()) && Modifier.isPublic(method.getModifiers())) {
            if (!checkConstraint(resource, method)) {
                ...
                type = handleReturnType(method);
                documentation = getDocumentation(method);
            }
        }
        return new ResourceMethod(method, type, documentation);
    }

    private MethodType handleReturnType(Method method) {
        if (method.getReturnType().equals(Void.TYPE)) {
            return MethodType.COMMAND;
        } else if (Resource.class.isAssignableFrom(method.getReturnType())) {
            if (method.getParameterTypes().length == 0) return MethodType.SUBRESOURCE;
            else return MethodType.ID_RESOURCE;
        } else {
